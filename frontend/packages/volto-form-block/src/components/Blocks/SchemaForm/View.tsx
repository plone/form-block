...
import { submitForm } from '@plone/volto-form-block/actions/form';
import ThankYou from '@plone/volto-form-block/components/ThankYou/ThankYou';
...
const FormBlockView = ({ data, id, path }: FormBlockViewProps) => {
  ...
  const lang = useSelector(
    (state: { intl: { locale: string } }) => state.intl.locale,
  );
  ...
  const onSubmit = async (formData) => {
    ...
    const captcha: { provider: any; props: any; token?: string } = {
    ...
    map(keys(submitData), (field) => {
      ...
      if (
        ...
        const time = submitData[field] as { hour: number; minute: number };
        submitData[field] = `${time.hour}:${time.minute}`;
      }
    ...
    // submitForm is a thunk that resolves once the form is submitted; the base
    // redux Dispatch type doesn't surface the returned promise.
    (
      dispatch(
        submitForm(path, id, submitData, captcha),
  ...
  const formatProperty = (factory, value) => {
    switch (factory) {
      ...
      case 'label_date_field':
        return isUndefined(value) ? '' : formatShortDate(value, lang);
      case 'label_datetime_field':
        return isUndefined(value) ? '' : formatLongDateTime(value, lang);
      default:
        return value;
    }
  };
