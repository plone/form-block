import React, { useEffect } from 'react';
import TextWidget from '@plone/volto/components/manage/Widgets/TextWidget';

import '@plone/volto-form-block/components/Widgets/HoneypotCaptchaWidget.css';

type HoneypotCaptchaWidgetProps = {
  id: string;
  value: string;
  onChange: (id: string, value: string) => void;
  onEdit?: boolean;
  captcha_props: { provider: string };
};

/* By default, captcha token is setted, and becames empty if user/bot fills the field. */
const HoneypotCaptchaWidget = ({
  id,
  value,
  onChange,
  onEdit,
  captcha_props,
}: HoneypotCaptchaWidgetProps) => {
  const title = '';
  const createToken = (id: string, value: string) => {
    const token = {
      id: id,
      value: value,
    };
    return JSON.stringify(token);
  };

  useEffect(() => {
    onChange('captchaToken', createToken(id, new Date().toString()));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  return onEdit ? (
    <></>
  ) : (
    <div className="honey-wrapper" key={'honeypot-captcha'}>
      <TextWidget
        id={captcha_props.provider}
        name={captcha_props.provider}
        label={title}
        title={title}
        onChange={onChange}
        value={value}
      />
    </div>
  );
};

export default HoneypotCaptchaWidget;
